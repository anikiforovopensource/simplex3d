 ...
 * @author Aleksey Nikiforov (lex)
 */
abstract class AnyQuat4[P] private[math] () extends MathObject {
 ...
 * @author Aleksey Nikiforov (lex)
 */
abstract class AnyMat[P] private[math] () extends MathObject {
  type Clone <: AnyMat[P]
  def columns: Int
  def rows: Int
 ...
 * @author Aleksey Nikiforov (lex)
 */
sealed trait AnyVec[P] extends MathObject {
  type Clone <: AnyVec[P]
  def components: Int
  def apply(i: Int) :P
