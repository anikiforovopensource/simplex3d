...
with ReadPropertyRef[Vec3i] with Cloneable[ReadVec3i] with Serializable
{
...
with PropertyRef[Vec3i] with Cloneable[Vec3i] with Serializable
{
  px = cx; py = cy; pz = cz
  ...
  def :=(r: Readable[Vec3i]) { val u = r.asInstanceOf[ReadVec3i]; x = u.x; y = u.y; z = u.z }
