...
sealed abstract class AnyMat2x3d extends Read2x3[ConstVec2d]
{
  ...
  final def /(s: Double) = { val inv = 1/s; new Mat2x3d(
    inv*m00, inv*m10,
    inv*m01, inv*m11,
    inv*m02, inv*m12
  )}
  ...
  final def invert() :Mat2x3d = inverse(this)

  final def ==(m: inMat2x3d) :Boolean = {
    if (m eq null) false
    else
      m00 == m.m00 && m10 == m.m10 &&
      m01 == m.m01 && m11 == m.m11 &&
  ...
  final def !=(m: inMat2x3d) :Boolean = !(this == m)
...
object ConstMat2x3d {
  ...
  def apply(m: ReadMat[_]) = new ConstMat2x3d(
    m.d00, m.d10,
    m.d01, m.d11,
    m.d02, m.d12
  )
...
) extends AnyMat2x3d with Mutable with Implicits[On]
{
  def *=(s: Double) {
    m00 *= s; m10 *= s;
    m01 *= s; m11 *= s;
...
object Mat2x3d {
  ...
  def apply(m: ReadMat[_]) = new Mat2x3d(
    m.d00, m.d10,
    m.d01, m.d11,
    m.d02, m.d12
  )
