...
sealed abstract class AnyMat4d extends Read4x4[ConstVec4d]
{
  ...
  private[math] final override def f33 = float(m33)
  ...
  final def !=(m: inMat4d) :Boolean = !(this == m)
...
) extends AnyMat4d with Mutable with Implicits[On] with Composite
{
