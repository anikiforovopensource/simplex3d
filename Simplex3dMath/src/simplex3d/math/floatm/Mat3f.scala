...
sealed abstract class AnyMat3f
{
    ...
    protected[math] def transposeMul(u: AnyVec3f) = new Vec3f(
...
object Mat3f {
    ...
    def apply(c0: Read3Double, c1: Read3Double, c2: Read3Double) = 
    new Mat3f(
    ...
    implicit def constToMutable(m: ConstMat3f) = Mat3f(m)
}
