...
extends ConstRotationSubMat2f with ReadFloatMat
{
    ...
    private[math] def mul(x: Float, y: Float, result: Vec3f) = {
    ...
    private[math] def transposeMul(x: Float, y: Float, z: Float, result: Vec2f) = {
    ...
    def *(u: AnyVec2f) = mul(u.x, u.y, new Vec3f)
