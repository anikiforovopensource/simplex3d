...
sealed abstract class AnyMat4x2f extends Read4x2[ConstVec4f]
{
  ...
  def *(u: inVec2f) = new Vec4f(
    m00*u.x + m01*u.y,
    m10*u.x + m11*u.y,
    m20*u.x + m21*u.y,
    m30*u.x + m31*u.y
  )
  private[math] def transposeMul(u: inVec4f) = new Vec2f(
    m00*u.x + m10*u.y + m20*u.z + m30*u.w,
    m01*u.x + m11*u.y + m21*u.z + m31*u.w
  )

  def ==(m: inMat4x2f) :Boolean = {
    if (m eq null) false
    else
      m00 == m.m00 && m10 == m.m10 && m20 == m.m20 && m30 == m.m30 &&
      m01 == m.m01 && m11 == m.m11 && m21 == m.m21 && m31 == m.m31
  ...
  def !=(m: inMat4x2f) :Boolean = !(this == m)
...
) extends AnyMat4x2f with Immutable
...
) extends AnyMat4x2f with Mutable with Implicits[On]
{
  def *=(s: Float) {
    m00 *= s; m10 *= s; m20 *= s; m30 *= s;
    m01 *= s; m11 *= s; m21 *= s; m31 *= s
...
object Mat4x2f {
  ...
  val Zero = ConstMat4x2f(0)
  val Identity = ConstMat4x2f(1)
