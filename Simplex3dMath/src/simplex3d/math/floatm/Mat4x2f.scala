...
extends ReadFloatMat
{
    ...
    private[math] def mul(x: Float, y: Float, result: Vec4f) = {
    ...
    private[math] def transposeMul(x: Float, y: Float, z: Float, w: Float, result: Vec2f) = {
    ...
    def *(u: AnyVec2f) = mul(u.x, u.y, new Vec4f)
