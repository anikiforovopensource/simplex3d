...
class Transform3dTest extends FunSuite {
    ...
    test("3D Transform") {
        ...
        for (i <- 0 until 100) {
            ...
            assert(Transform3(Mat3f(m3)).toMatrix() == Mat3x4(Mat3f(m3)))

            val m3x4 = Mat3x4(r, r, r, r, r, r, r, r, r, r, r, r)
            assert(Transform3(m3x4).toMatrix() == m3x4)
            assert(Transform3(Mat3x4f(m3x4)).toMatrix() == Mat3x4(Mat3x4f(m3x4)))
        ...
        def test(t: Transform3) {
            ...
            assert(approxEqual(t.invert().toMatrix, inverse(t.toMatrix), 1e-12f))
