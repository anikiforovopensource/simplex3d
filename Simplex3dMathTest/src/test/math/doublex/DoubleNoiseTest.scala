 ...
 */
class DoubleNoiseTest extends FunSuite {
  ...
  test("Noise") {
    ...
    var i = 0; while(i < 1000) {
      ...
      val p2 = n.noise2(u)
      if (approxEqual(p2.x, p2.y, 1e-3)) count += 1

      val p3 = n.noise3(u)
      if (approxEqual(p3.x, p3.y, 1e-3)) count += 1
      if (approxEqual(p3.y, p3.z, 1e-3)) count += 1

      val p4 = n.noise4(u)
      if (approxEqual(p4.x, p4.y, 1e-3)) count += 1
      if (approxEqual(p4.y, p4.z, 1e-3)) count += 1
      if (approxEqual(p4.z, p4.w, 1e-3)) count += 1
  ...
  test("Noise Sum") {
    ...
    test2dNoise(0, 0, noise(_))
    test3dNoise(0, 0, noise(_))
    test4dNoise(0, 0, noise(_))
  }
  ...
  test("Tiled Noise sum") {
    ...
    test1dTiles(2, noise(_))
    test2dTiles(Vec2(2), noise(_))
    test3dTiles(Vec3(2), noise(_))
    test4dTiles(Vec4(2), noise(_))
  }
  ...
  test("Noise serialization") {
    ...
    val noiseSum0 = new NoiseSum(2.0, 5, 2.1, 0.4, new ClassicalGradientNoise(11))
    val turbulence0 = new Turbulence(2.0, 5, 2.1, 0.4, 0.2, new ClassicalGradientNoise(11))
    val tiledNoiseSum0 = new TiledNoiseSum(Vec4(3), 2.0, 5, 2.1, 0.4, new ClassicalGradientNoise(11))
    val tiledTurbulence0 = new TiledTurbulence(Vec4(3), 2.0, 5, 2.1, 0.4, 0.2, new ClassicalGradientNoise(11))
